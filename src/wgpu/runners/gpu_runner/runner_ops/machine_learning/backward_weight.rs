...
impl GpuRunner{
    ...
    ///
    ///     let full_gpu_output: Vec<Tensor<f32>> = runner.backward_weight_no_activ().await;
    /// }
    /// ```
    pub async fn backward_weight_no_activ(&mut self) -> Vec<Tensor<f32>>{
        ...
        let mut return_vec: Vec<Tensor<f32>> = self.run_ops(&GpuOperations::BackwardWeightNoActiv).await;
    ...
    /// Backpropagation for weights with relu activation
    /// No need to care about output shape while creating sample
    /// It is managed by GpuRunner
    ///
    /// # Example
    ...
    ///     let learning_rate = 0.01;
    ///
    ///     let sample = Sample::from_data(vec!{weights, grad_output, linear_cache, relu_cache}, vec!{learning_rate}, &[]);
    ///
    ///     let mut runner = GpuRunner::init(1, MemoryMetric::GB);
    ///
    ///     runner.append(sample);
    ///
    ///     let full_gpu_output: Vec<Tensor<f32>> = runner.backward_weight_relu().await;
    /// }
    /// ```
    pub async fn backward_weight_relu(&mut self) -> Vec<Tensor<f32>>{
        ...
        let mut return_vec: Vec<Tensor<f32>> = self.run_ops(&GpuOperations::BackwardWeightRelu).await;
    ...
    ///     let learning_rate = 0.01;
    ///
    ///     let sample = Sample::from_data(vec!{weights, grad_output, linear_cache, sigmoid_cache}, vec!{learning_rate}, &[]);
    ///
    ///     let mut runner = GpuRunner::init(1, MemoryMetric::GB);
    ///
    ///     runner.append(sample);
    ///
    ///     let full_gpu_output: Vec<Tensor<f32>> = runner.backward_weight_sigmoid().await;
    /// }
    /// ```
    pub async fn backward_weight_sigmoid(&mut self) -> Vec<Tensor<f32>>{
        ...
        let mut return_vec: Vec<Tensor<f32>> = self.run_ops(&GpuOperations::BackwardWeightSigmoid).await;
